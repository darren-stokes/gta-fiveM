...
-- There's a newer way that ESX interacts with limits which broke this script
-- I've changed it that instead of communicating with the database, we check here for carry limits
-- These are defined here. Don't change anything else in this script, only these variables
weed_limit = 100 -- Weed
coke_limit = 100 -- Cocaine
meth_limit = 100 -- Meth
opium_limit = 100 -- Opium
moons_limit = 100 -- Straight Moonshne
...
local function HarvestOpium(source)
	...
	SetTimeout(5000, function()
		...
		if PlayersHarvestingOpium[source] == true then
			...
			if opium.count >= opium_limit then
...
local function HarvestMoonshine(source)
	...
	SetTimeout(5000, function()
		...
		if PlayersHarvestingMoonshine[source] == true then
			...
			if moonshine.count >= moons_limit then
